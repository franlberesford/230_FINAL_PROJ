...
mcmc_alg_meth_1 <- function(num_locs, num_its, dist_mat, X, t_cov, abc_delta,  beta_prop_sigma, theta_prop_sigma){
  ...
  for (t in 2:num_its){
    ...
    ### recalculate ABC based on new beta values 
    ABC <- find_abc(y_sample[t-1,], delta =abc_delta )
    ...
    acc_prob_beta <- exp(prior_beta - prop_beta) * t(t_cov)%*% exp(y_sample[t,]) + (prior_beta - prop_beta)* t(log(t_cov))%*% X
    ...
    acc_prob_theta <- 0.5*(-t(y_sample[t,])%*%solve(temp_Sigma_prime)%*%y_sample[t,] +  t(y_sample[t,])%*%solve(temp_Sigma_prior)%*%y_sample[t,] - log( det_temp_Sigma_prime) + log(det_temp_Sigma_prime ) ) 
