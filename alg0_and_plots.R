...
alg0_and_plots <- function(num_locs=nlocs, num_its= 1000, dist_mat = distmat, X=Xs, t_cov= ts, abc_delta= 1e-30,beta_prop_sigma = 0.01, theta_prop_sigma = c(1,5)){
  test <- mcmc_alg_meth_0(num_locs, num_its= num_its, dist_mat = distmat, X=Xs, t_cov= ts, abc_delta,beta_prop_sigma , theta_prop_sigma)
